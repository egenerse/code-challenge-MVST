...
export default function Repositories({ repositories }: RepositoriesProps) {
  ...
  return (
    <Box sx={{ display: 'flex', flex: 8, flexDirection: 'column', flexWrap: 'wrap' }}>
      ...
      {allFilteredRepositories?.length === 0 ? <Typography>{username} doesn’t have any repositories that match.</Typography> :
        <>
          {paginationCount > 1 && (
            <Pagination
            ...
            {paginatedData?.map((repo, index) => (
              <Box key={repo.name} >
                <ListItem sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'flex-start' }}>
                  <Link to={`${repo.url}`} target="_blank">{repo.name}</Link>
                  <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                    {repo.languages.nodes.map(language => <Typography key={`${repo.name}-${language.name}`}>⏺ {language.name}</Typography>)}
                  </Box>
                </ListItem>
